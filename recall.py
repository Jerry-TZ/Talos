...
# 改成 `[\W_]`:凡是**不是词字符**的都算子句开头。Python 的 `\w` 在 str 模式下含 CJK,
# 所以 `\W` 正好等于「全角半角所有标点 + 空白」,一次覆盖完,以后不用再回来加。
# 补个 `_` 是因为下划线算 `\w` —— 而它显然不是词的一部分,这是 `[\W_]` 这个惯用写法
# 存在的全部理由(测试拿 `string.punctuation` 整扫时它是唯一漏网的那个)。
# 留下的边界:`这条不用于生产` 这种紧贴汉字的写法仍然不摘 —— 跟上一版一致,不是回归。
_DONT_USE = re.compile(r"(?:^|[\W_])[ \t]*不用于[:：]?[^\n]*", re.MULTILINE)
...
def _activate(nodes: list, edges: dict, query: str) -> dict:
    ...
    # `tests/test_memory.py::test_nodes_that_share_nothing_with_the_query_must_not_change_the_answer`
    # ——`xfail(strict=True)`,现在是红的,修好了会因为"意外通过"再红一次,挂账不会烂掉。
    # 它判的是**不变性**:与查询零交集的节点不该改变任何结果。不变性不需要标注,
    # 所以「没有数据不能调参」拦不住它 —— 上一版拿那句话把整件事挡回去了,挡错了。
    #
    # **上一版写的触发条件是错的**,原话是「哪天量到分数普遍越过 1……那时候必须归一化」。
    # 实测:加入与查询零交集的近重复节点,top-5 身份在**全场最高分 0.33** 时就翻了,
    # 离 1.0 还远。绝对门槛(`THRESH` / `BODY_FLOOR`)确实绑在量纲上,但失真比量纲越界早得多。
    #
