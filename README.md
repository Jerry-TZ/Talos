...
**一个你能完整读完的编程 agent。** 4074 行 Python,223 条离线判据,一份不糊弄人的安全说明。
...
| 文件 | 行数 | 职责 |
|---|---|---|
| `agent.py` | 3063 | 循环 + 工具 + 权限门 + 自学习 |
| `console_ui.py` | 214 | 终端界面(可整体替换) |
| `recall.py` | 526 | 联想记忆:扩散激活检索 |
...
- **能验证** — 223 个测试,**离线、免 API key、几秒跑完**,CI 在 Linux/Windows × Python 3.10/3.13 上都跑。clone 下来立刻知道它没坏。
...
.venv\Scripts\python.exe -m pytest tests/ -q     # 223 条,约 8 秒,不联网、不需要 key
