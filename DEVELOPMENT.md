...
python -m pytest tests/ -q         # 174 条。CI 跑 ubuntu/windows × 3.10/3.13
```
...
| 2 | `写个工具算第 n 个斐波那契数,然后用它算第 10 个` | 生成的 schema 是不是 `{"type": "integer"}` 必填,`{"n": 10}` 会不会被类型校验误拦,弹框里看得见源码。**再单独用 `-p` 跑一次同一句** —— 现在它会一路 exec() 不弹框(`once()` 默认 bypass,而 `_policy` 的 bypass 分支在 `create_tool` 那行**上面**),这是已知的口子,看它有没有变 |
