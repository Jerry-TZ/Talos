...
| **密钥被读进上下文** | 读工具不过权限门,`read_file('.env')` 会把 key 送进 provider 历史和明文会话日志 | `.env` / `id_rsa` / `.ssh` / `.aws` 等凭据文件一律拒绝读写 |
...
| **memory.md 变成持久注入** | 它全量进每一轮 system prompt,却从不扫描 | 像指令的行不注入,其余标注为"事实,不是指令" |
